...
Public Class Form_devolucionescompras
    ...
    Private Sub Form_devolucionescompras_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        datagrid_devoluciones.BringToFront()
        datagrid_devoluciones.Size = New Size(914, 521)
    End Sub

    Private Sub Form_devolucionescompras_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        ...
        Try
            ...
            Me.ComboBox_cta_caja_banco.ValueMember = "cuentapuc"
    ...
    Private Sub load_grid_PEDIDOS_CLIENTES()
        ...
        Try
            sql = "select * from devolucionescompras where month(STR_TO_DATE(fecha,'%d/%m/%Y'))='" & mes_num_text_general & "' and year(STR_TO_DATE(fecha,'%d/%m/%Y'))='" & NumericUpDown_anno.Value & "'"
            ...
            Me.datagrid_devoluciones.Columns(2).HeaderText = "Factura"
            Me.datagrid_devoluciones.Columns(2).Width = 90
            Me.datagrid_devoluciones.Columns(2).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
            ...
            Me.datagrid_devoluciones.Columns(16).Visible = False
            Me.datagrid_devoluciones.Columns(17).Visible = False
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
        conex.Close()
    ...
    Private Sub load_grid_PEDIDOS_CLIENTES_FILTRO(ByRef filtro_estado As String)
        Try
            sql = "select * from devolucionescompras where tipodev='" & tipo_devolucion & "' and estado='" & filtro_estado & "'"
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            da.Fill(dt)
            Me.datagrid_devoluciones.DataSource = dt
    ...
    Private Sub Button_guardar_Click(sender As Object, e As EventArgs) Handles Button_guardar.Click
        ...
        If Me.Button_guardar.Text = "Guardar" Then
            ...
            sql = "INSERT INTO devolucionescompras(FECHA, factura, codcliente, cliente, Observaciones, empleadocod, empleado, Estado, nrofac, tipodev)" &
        ...
        If Me.Button_guardar.Text = "Guardar Cambios" And Label_estado_dev.Text = "PENDIENTE" Then
            sql = "UPDATE devolucionescompras SET OBSERVACIONES='" & Text_leyenda.Text & "' WHERE CONS=" & CLng(Label_consecutivo_dev.Text) & ""
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            Try
                da.Fill(dt)
    ...
    Private Sub loads_productos()
        ...
        Try
            sql = "SELECT producto, codprod FROM compras_prods where documento='" & Me.Label_Nofactura.Text & "'"
            da = New MySqlDataAdapter(sql, conex)
    ...
    Private Sub Button_agregar_prod_Click(sender As Object, e As EventArgs) Handles Button_agregar_prod.Click
        ...
        Me.Cursor = Cursors.WaitCursor
        If PROD_DECIMAL = "NO" Then sql = "INSERT INTO devolucionescompras_prods (documento, codprod, producto, cantidad, base, imp, IMP2, impuesto, IMPUESTO2, valoru, valortotal, estado,  inventariado, IMPNOM1, IMPNOM2, DECI, COSTO)" &
        ...
        If PROD_DECIMAL = "SI" Then sql = "INSERT INTO devolucionescompras_prods (documento, codprod, producto, cantidad, base, imp, IMP2, impuesto, IMPUESTO2, valoru, valortotal, estado,  inventariado, IMPNOM1, IMPNOM2, DECI, COSTO)" &
    ...
    Private Sub Btn_Ver_editar_Click(sender As Object, e As EventArgs) Handles Btn_Ver_editar.Click
        ...
        Try
            sql = "SELECT * FROM devolucionescompras WHERE CONS = " & CodDocumento & ""
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            da.Fill(dt)
        ...
        'datos de factura
        Try
            sql = "SELECT documento, fecha, total, DEVOLUCION, mediodepago FROM compras WHERE documento = " & Label_Nofactura.Text & ""
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            da.Fill(dt)
            For Each row As DataRow In dt.Rows
...
where medios_pago.mediopago='" & medio_de_pago & "'"
    ...
    Private Sub button_Quitar_Prod_Click(sender As Object, e As EventArgs) Handles button_Quitar_Prod.Click
        ...
        If RTA = 6 Then
            sql = "delete from devolucionescompras_prods where cons=" & CInt(prod_cod) & ""
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            Try
                da.Fill(dt)
    ...
    Private Sub Button_EXPORTAR_Click(sender As Object, e As EventArgs) Handles Button_EXPORTAR.Click
        Try
            sql = "SELECT CODPROD, PRODUCTO, CANTIDAD, VALORU, VALORTOTAL FROM devolucionescompras_PRODS WHERE documento = " & CInt(Label_consecutivo_dev.Text) & ""
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            da.Fill(dt)
        ...
        If RTA = 6 Then
            ...
            Try
                For J As Integer = 0 To Me.datagrid_productos.RowCount - 1
                    ...
                    sql = "INSERT INTO bodega_principal (fecha, CodBodega, NomBodega, TipoDocumento, Documento, Rol, CodProducto, Producto, SALDOANT, Entran, Salen, Saldo, valoru, valortotal, costo, Empleado, Estado)" &
            ...
            If ComboBox_cta_caja_banco.Text <> "" Then
                MEDIO_PAGO_DESTINO_NOMBRE = ComboBox_cta_caja_banco.SelectedValue.ToString
                Dim pago() As String
                ...
                sql = "INSERT INTO cajaspuc(Codcuenta, cuenta, TERCERO, Fecha, Documento, TipoDoc, Rol, SaldoAnt, debe, haber, Saldo, DETALLE, Estado, annoactual)" &
       "VALUES ('" & pago(0) & "','" & pago(1) & "','" & TXT_DOC_CLIENTE.Text & "|" & TXT_NOM_CLIENTE.Text & "','" & DateTime.Now.ToShortDateString & "','" & consecutivo & "','DEVOLUCION DE VENTA','SALEN'," & saldo & "," & PROD_ENTRAN & "," & PROD_SALEN & "," & PROD_SALDO & ",'DEVOLUCION DE VENTA','DESCARGADO','" & comex_annoactual & "')"
                da = New MySqlDataAdapter(sql, conex)
                dt = New DataTable
                Try
                    da.Fill(dt)
            ...
            sql = "update devolucionescompras SET Descargado='SI', estado='FINALIZADO', mediopago='" & ComboBox_cta_caja_banco.SelectedValue & "', vrpago='" & TextBoxTotal_Dev.Text & "' WHERE CONS=" & CLng(Label_consecutivo_dev.Text) & ""
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            Try
                da.Fill(dt)
    ...
    Private Sub GENERAR_VENTA()
        ...
        sql = "UPDATE DEVOLUCIONEScompras set FACTURADO='SI', nrofac='" & Label_Nofactura.Text & "' WHERE CONS=" & CLng(Label_consecutivo_dev.Text) & ""
        da = New MySqlDataAdapter(sql, conex)
        dt = New DataTable
        Try
            da.Fill(dt)
    ...
    Private Sub Button_crear_dev_Click(sender As Object, e As EventArgs) Handles Button_crear_dev.Click
        ...
        cmd.CommandText = "select max(cons) + 1 from devolucionescompras"
        conex.Open()
        Try
        ...
        sql = "UPDATE compras set devolucion='" & consecutivo & "' WHERE CONS=" & ID_VENTA_dev & ""
        da = New MySqlDataAdapter(sql, conex)
        dt = New DataTable
        Try
            da.Fill(dt)
...
where medios_pago.mediopago='" & medio_depago_dev & "'"
    ...
    Private Sub GRID_VENTAS_fecha_dev()
        Try
            sql = "select Documento, Fecha, Doccliente, ClienteNom, Total, devolucion from compras WHERE month(STR_TO_DATE(fecha,'%d/%m/%Y'))='" & mes_num_text_general & "'"
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            da.Fill(dt)
            Me.datagrid_fac_dev.DataSource = dt
            Me.datagrid_fac_dev.Columns(0).HeaderText = "Factura"
            Me.datagrid_fac_dev.Columns(0).Width = 70
            Me.datagrid_fac_dev.Columns(0).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft
            Me.datagrid_fac_dev.Columns(1).HeaderText = "Fecha"
            Me.datagrid_fac_dev.Columns(1).Width = 70
            Me.datagrid_fac_dev.Columns(1).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft
            Me.datagrid_fac_dev.Columns(2).HeaderText = "Doc. Cliente"
            Me.datagrid_fac_dev.Columns(2).Width = 130
            Me.datagrid_fac_dev.Columns(2).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft
            Me.datagrid_fac_dev.Columns(3).HeaderText = "Cliente"
            Me.datagrid_fac_dev.Columns(3).Width = 280
            Me.datagrid_fac_dev.Columns(3).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft
            Me.datagrid_fac_dev.Columns(4).HeaderText = "Totalventa"
            Me.datagrid_fac_dev.Columns(4).Width = 100
            Me.datagrid_fac_dev.Columns(4).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft
            Me.datagrid_fac_dev.Columns(5).Visible = False
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
        conex.Close()
    ...
    Private Sub GRID_VENTAS_nro_dev()
        Try
            sql = "select Documento, Fecha, Doccliente, ClienteNom, Total, devolucion from compras WHERE documento='" & TextBox_norFACDEV.Text & "'"
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            da.Fill(dt)
            Me.datagrid_fac_dev.DataSource = dt
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        Me.Cursor = Cursors.WaitCursor
        sql = "INSERT INTO devolucionescompras_prods (documento, producto, cantidad, base, imp, IMP2, impuesto, IMPUESTO2, valoru, valortotal, estado)" &
        ...
        If Checkbox_aplicar_fac.Checked = True Then
            sql = "insert into compras_prods (documento,codprod, producto, valortotal, estado) values('" & Me.ComboBox_factura_nota_dev.SelectedValue.Text & "','','Nota de Descuento " & Label_consecutivo_dev.Text & "','" & CInt(TextBox_valor_desc.Text) * -1 & "','DESCARGADO')"
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            Try
                da.Fill(dt)
    ...
    Private Sub Checkbox_aplicar_fac_OnChange(sender As Object, e As EventArgs) Handles Checkbox_aplicar_fac.OnChange
        If Checkbox_aplicar_fac.Checked = True Then
            Try
                sql = "select * from compras WHERE month(STR_TO_DATE(fecha,'%d/%m/%Y'))='" & mes_num_text_general & "' and doccliente='" & TXT_DOC_CLIENTE.Text & "'"
    ...
    Private Sub COMBO_NOM_CLIENTE_FILTRO_SelectedIndexChanged(sender As Object, e As EventArgs)
    ...
    Private Sub TextBox_imp_dev_TextChanged(sender As Object, e As EventArgs) Handles TextBox_imp_dev.TextChanged
    ...
    Private Sub TextBoxTotal_Dev_TextChanged(sender As Object, e As EventArgs) Handles TextBoxTotal_Dev.TextChanged
    ...
    Private Sub load_grid_EXPORTAR()
        Try
            sql = "select CONS, FECHA, CODCLIENTE, CLIENTE, ESTADO, DESCARGADO, FACTURADO, NROFAC from DEVOLUCIONEScompras WHERE ESTADO='" & filtro & "'"
            If filtro = "TODO" Then sql = "select CONS, FECHA, CODCLIENTE, CLIENTE, ESTADO, DESCARGADO, FACTURADO, NROFAC from DEVOLUCIONEScompras"
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            da.Fill(dt)
            Me.datagrid_devoluciones.DataSource = dt
    ...
    Private Sub grid_comprasprods()
        Try
            sql = "SELECT * FROM DEVOLUCIONEScompras_PRODS WHERE documento = " & CInt(Label_consecutivo_dev.Text) & ""
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            da.Fill(dt)
    ...
    Private Sub datos_del_prod_en_factura()
        Try
            sql = "SELECT * FROM compras_prods where documento=" & CInt(Label_Nofactura.Text) & " and codprod=" & Me.combobox_BuscarProd.SelectedValue & ""
            da = New MySqlDataAdapter(sql, conex)
            dt_saldos = New DataTable
            da.Fill(dt_saldos)
            For Each row As DataRow In dt_saldos.Rows
    ...
    Private Sub COMBO_DOC_CLIENTE_FILTRO_SelectionChangeCommitted(sender As Object, e As EventArgs)
        Timer_clientes.Enabled = True

    End Sub

    Private Sub COMBO_NOM_CLIENTE_FILTRO_SelectionChangeCommitted(sender As Object, e As EventArgs)
        Timer_clientes.Enabled = True
    ...
    Private Sub TextBox_imp_dev_KeyPress(sender As Object, e As KeyPressEventArgs) Handles TextBox_imp_dev.KeyPress
        e.KeyChar = ""
