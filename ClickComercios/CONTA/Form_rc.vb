...
Public Class Form_rc
    ...
    Private Sub Form_rc_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Button_guardar_Click(sender As Object, e As EventArgs) Handles Button_guardar.Click
        ...
        If ComboBox_tipo_ingreso.Text = Nothing Then MsgBox("Seleccione un concepto.", vbInformation) : Exit Sub
        If ComboBox_MEDIOPAGO.Text = Nothing Then MsgBox("Seleccione un medio de pago.", vbInformation) : Exit Sub
        If ComboBox_cta_caja_banco.Text = Nothing Then MsgBox("Seleccione una cuenta.", vbInformation) : Exit Sub
        ...
        If TextBox_referencia.Text = "" Then MsgBox("Digite el No. de referencia.", vbInformation) : Exit Sub

        If TextBox_valorINGRESO.Text = "" Or TextBox_valorINGRESO.Text = "0" Then MsgBox("Digite el valor.", vbInformation) : Exit Sub
        ...
        Dim RTA = MsgBox("Seguro desea registrar el Ingreso.? " & Chr(13) &
                         ...
                         TextBox_concepto.Text & Chr(13) &
                         "$ " & TextBox_valorINGRESO.Text, MsgBoxStyle.Question + MsgBoxStyle.YesNo)
        If RTA = "6" Then
            ...
            Me.TextBox_valorINGRESO.Enabled = False
            Me.TextBox_referencia.Enabled = False
            Me.TextBox_concepto.Enabled = False
            ...
            sql = "INSERT INTO recibos_CAJA(documento, fecha, DocCliente, ClienteNom, Clientetel, clientedir, descripcion, referencia, valor, empleadocod, empleado, turno, Estado, concepto, cuenta, mediodepago, cuentapago, imp, impvr, rete, retevr) 
VALUES(" & CLng(consecutivo) & ",'" & turno_actualfecha & "','" & CStr(TXT_DOC_CLIENTE.Text) & "','" & TXT_NOM_CLIENTE.Text & "','" & TXT_TELS_CLIENTE.Text & "','" & TXT_DIR_CLIENTE.Text & "','" & TextBox_concepto.Text & "','" & TextBox_referencia.Text & "'," & CInt(TextBox_valor.Text) & "," & CStr(usrdoc) & ",'" & usrnom & "','" & turno_actual_global & "','DESCARGADO','" & CStr(ComboBox_tipo_ingreso.Text) & "','" & cod_cuenta_puc & "|" & nom_cuenta_puc & "','" & CStr(ComboBox_MEDIOPAGO.Text.ToUpper) & "','" & CStr(ComboBox_cta_caja_banco.SelectedValue) & "|" & CStr(ComboBox_cta_caja_banco.Text) & "','" & CStr(ComboBox_impuestos.Text) & "','" & CStr(TextBox_impuestos.Text) & "','" & CStr(ComboBox_RETENCIONES.Text) & "','" & CStr(TextBox_retenciones.Text) & "')"
            ...
            ' actualizo cuenta X COBRAR
            If ComboBox_cxc.Text <> "" Then
                sql = "UPDATE carteracredito SET saldo='" & CInt(Label_SALDO_actual.Text) - CInt(TextBox_valor.Text) & "' 
where tipo='CARTERA' and CONSECUTIVO='" & ComboBox_cxc.SelectedValue & "'"
                da = New MySqlDataAdapter(sql, conex)
                dt = New DataTable
                Try
                    da.Fill(dt)
...
where tipo='CARTERA' and CONSECUTIVO='" & ComboBox_cxc.SelectedValue & "'"
            ...
            If PERMISO__general(6) = "SI" Then
                ...
                sql = "INSERT INTO CAJA(turno, fecha, documento, tipoDocumento, subtotal, descuento, total, MEDIODEPAGO, EmpleadoDoc, empleado, estado)" &
                      "VALUES ('" & turno_actual_global & "','" & turno_actualfecha & "'," & consecutivo & ",'RECIBO DE CAJA'," & CInt(Me.TextBox_valorINGRESO.Text) & ",0," & CInt(Me.TextBox_valorINGRESO.Text) & ",'" & ComboBox_MEDIOPAGO.Text.ToUpper & "'," & CInt(usrdoc) & ",'" & usrnom & "','DESCARGADO')"
                da = New MySqlDataAdapter(sql, conex)
                dt = New DataTable
                Try
                    da.Fill(dt)
            ...
            PROD_SALEN = 0  ' TOMO LA CANTIDAD DEL GRID Q ESTA RECORRIENDO
            PROD_ENTRAN = CInt(TextBox_valorINGRESO.Text) ' POR QUE QUE ES UNA venta
            'REGISTRO EN PUC

            sql = "INSERT INTO cajaspuc(Codcuenta, cuenta, TERCERO, Fecha, Documento, TipoDoc, Rol, debe, haber, detalle, Estado, annoactual)" &
               "VALUES ('" & ComboBox_cta_caja_banco.SelectedValue & "','" & ComboBox_cta_caja_banco.Text & "','" & CStr(TXT_DOC_CLIENTE.Text & "|" & TXT_NOM_CLIENTE.Text) & "','" & turno_actualfecha & "','" & consecutivo & "','RECIBO DE CAJA','ENTRAN'," & PROD_ENTRAN & "," & PROD_SALEN & ",'" & CStr(ComboBox_tipo_ingreso.Text & " | " & TextBox_concepto.Text) & "','DESCARGADO','" & comex_annoactual & "')"
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            Try
                da.Fill(dt)
            ...
            MsgBox("Se Registró el Comprobante de Ingreso.", vbInformation)
    ...
    Public Sub ExportarDatosPDF_POS(ByVal document As Document)
        ...
        Dim encabezado_OBS As New Paragraph("OBSERVACION: " & Chr(13) & TextBox_concepto.Text, FontFactory.GetFont(FontFactory.HELVETICA_BOLD, 6D, iTextSharp.text.Font.BOLD)) : encabezado91.Alignment = Element.ALIGN_LEFT
        ...
        celltotal.Phrase = New Phrase(FormatNumber(TextBox_valorINGRESO.Text, 0), FontFactory.GetFont(FontFactory.COURIER_BOLD, 10, iTextSharp.text.Font.NORMAL))
    ...
    Private Sub Timer_MEDIO_PAGO_Tick(sender As Object, e As EventArgs) Handles Timer_MEDIO_PAGO.Tick
        '        Me.Timer_MEDIO_PAGO.Enabled = False


        '        Try
        '            sql = "SELECT * FROM medios_pago
        'inner join cajasybancos on medios_pago.codcuenta=cajasybancos.cons
        ' WHERE medios_pago.CONS='" & Me.ComboBox_MEDIOPAGO.SelectedValue.ToString & "'"
        '            da = New MySqlDataAdapter(sql, conex)
        '            dt = New DataTable
        '            da.Fill(dt)
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        Try
            sql = "SELECT concat(concepto,' ',descripcion,' Ref:',referencia) as Concepto FROM recibos_caja WHERE CONS = " & CInt(Label_consecutivo.Text) & ""
            da = New MySqlDataAdapter(sql, conex)
        ...
        Try
            ...
            Dim filename As String = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + "\MiClickDerecho Docs\Recibo de Caja " & Label_consecutivo.Text & ".pdf"
            ...
            Process.Start("C:\Program Files\Tracker Software\PDF Viewer\pdfxcview.exe", """" & Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + "\MiClickDerecho Docs\Recibo de Caja " & Label_consecutivo.Text & ".pdf" & """")
    ...
    Public Sub ExportarDatosPDF(ByVal document As Document)
        ...
        cell_PIE.HorizontalAlignment = Element.ALIGN_RIGHT
        tabla_PIE.AddCell(cell_PIE)
        cell_PIE.Phrase = New Phrase(FormatNumber(TextBox_valorINGRESO.Text, 2), totaltotalFont)
        cell_PIE.HorizontalAlignment = Element.ALIGN_RIGHT
        cell_PIE.BackgroundColor = BaseColor.LIGHT_GRAY
        tabla_PIE.AddCell(cell_PIE)
        tabla_PIE.CompleteRow()
    ...
    Private Sub Timer_VER_Tick(sender As Object, e As EventArgs) Handles Timer_VER.Tick
        ...
        Try
            sql = "SELECT * FROM recibos_CAJA where DOCUMENTO=" & ID_rc_VER & ""
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            da.Fill(dt)
            For Each row As DataRow In dt.Rows
                ...
                TextBox_concepto.Text = row.Item("descripcion")
                ...
                ComboBox_tipo_ingreso.Text = row.Item("concepto")
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        ...
        Process.Start("C:\Program Files\Tracker Software\PDF Viewer\pdfxcview.exe", " /print:printer=""" & impresosaPredt & """ """ & Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + "\MiClickDerecho Docs\Recibo de Caja " & Label_consecutivo.Text & ".pdf" & """")
    ...
    Private Sub ComboBox1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBox_tipo_ingreso.SelectedIndexChanged

    End Sub

    Private Sub TextBox_DOC_CLIENTE_SelectedValueChanged(sender As Object, e As EventArgs)
    End Sub
    ...
    Private Sub ComboBox2_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBox_cxc.SelectedIndexChanged
    ...
    Private Sub Btn_nuevo_mov_Click(sender As Object, e As EventArgs) Handles Btn_nuevo_mov.Click
        ...
        cmd.CommandText = "select max(documento) + 1 from recibos_caja"
        conex.Open()
        Try
    ...
    Private Sub Timer_factura_info_Tick(sender As Object, e As EventArgs) Handles Timer_factura_info.Tick
        ...
        Try
            sql = "SELECT * FROM carteracredito where tipo='CARTERA' and CONSECUTIVO='" & ComboBox_cxc.SelectedValue & "'"
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            da.Fill(dt)
            ...
            For Each row As DataRow In dt.Rows
                ...
                TextBox_concepto.Text = CStr(row.Item("concepto"))
        ...
        Try
            sql = "SELECT * from recibos_caja where REFERENCIA='" & TextBox_referencia.Text & "' and concepto='Cuenta Por Cobrar' and estado='DESCARGADO'"
    ...
    Private Sub ComboBox1_SelectionChangeCommitted(sender As Object, e As EventArgs) Handles ComboBox_tipo_ingreso.SelectionChangeCommitted
        ...
        ComboBox_cxc.DataSource = Nothing
        ...
        If ComboBox_tipo_ingreso.Text = "Cuenta Por Cobrar" Then
            Try
                sql = "SELECT consecutivo, CONCAT('CxC-',CONSECUTIVO,' de ',FECHA, ' Factura-',documento,' | $ ', replace(format(Valor,0),',','.')) AS FACTURA, saldo FROM carteracredito WHERE tipo='CARTERA' AND CLIENTEDOC='" & TXT_DOC_CLIENTE.Text & "' AND ESTADO='PENDIENTE'"
                da_CXC = New MySqlDataAdapter(sql, conex)
                dt_CXC = New DataTable
                da_CXC.Fill(dt_CXC)
                Me.ComboBox_cxc.DataSource = dt_CXC.DefaultView
                Me.ComboBox_cxc.DisplayMember = "FACTURA"
                Me.ComboBox_cxc.ValueMember = "CONSECUTIVO"
            Catch ex As Exception
                MsgBox(ex.Message)
            End Try
            conex.Close()
            ...
            ComboBox_cxc.SelectedItem = Nothing


            Label5.Visible = True
            ComboBox_cxc.Visible = True
        ...
        'TextBox_referencia.Enabled = True
        'TextBox_concepto.Enabled = True
        'ComboBox_cxc.Enabled = False
        'TextBox_valorINGRESO.Enabled = True
    End Sub
    Private Sub Timer_cuentapuc_concepto_Tick(sender As Object, e As EventArgs) Handles Timer_cuentapuc_concepto.Tick
        ...
        If Label_estado_rc.Text = "DESCARGADO" Then Exit Sub

        If ComboBox_tipo_ingreso.Text.Contains("|") = True Then
            cuenta_puc = Split(ComboBox_tipo_ingreso.Text, "|")
            Exit Sub
        End If


        Try
            sql = "SELECT * FROM data_docs WHERE CONS='" & ComboBox_tipo_ingreso.SelectedValue & "'"
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            da.Fill(dt)
            For Each row As DataRow In dt.Rows
        ...
        da.Dispose()
        dt.Dispose()
        If ComboBox_tipo_ingreso.SelectedValue = Nothing Then Exit Sub
    ...
    Private Sub Button_anular_Click(sender As Object, e As EventArgs) Handles Button_anular.Click
        ...
        Select Case MessageBox.Show("Confirma la anulación del comprobante?", "Anular Factura.", MessageBoxButtons.YesNo, MessageBoxIcon.Information)
            Case DialogResult.Yes

                sql = "UPDATE RECIBOS_caja SET estado='ANULADO' WHERE DOCUMENTO=" & CInt(ID_VENTA_VER) & ""
                da = New MySqlDataAdapter(sql, conex)
                dt = New DataTable
                Try
                    da.Fill(dt)
                ...
                sql = "delete from CAJASPUC WHERE DOCUMENTO=" & CInt(Label_consecutivo.Text) & " and tipodoc='RECIBO DE CAJA'"
                da = New MySqlDataAdapter(sql, conex)
                dt = New DataTable
                Try
                    da.Fill(dt)
                ...
                'CAJA EMPLEADO
                sql = "UPDATE CAJA SET ESTADO='ANULADO' WHERE DOCUMENTO=" & CInt(Label_consecutivo.Text) & " and tipodocumento='RECIBO DE CAJA'"
                da = New MySqlDataAdapter(sql, conex)
                dt = New DataTable
                Try
                    da.Fill(dt)
    ...
    Private Sub Button_REFRESH_KeyPress(sender As Object, e As KeyPressEventArgs) Handles Button_REFRESH.KeyPress
    ...
    Private Sub totalizar()
        ...
        TextBox_subtotal.Text = FormatNumber(TextBox_valor.Text, 2)

        TextBox_valorINGRESO.Text = CDec(TextBox_subtotal.Text) + CDec(TextBox_impuestos.Text) - CDec(TextBox_retenciones.Text)
        TextBox_valorINGRESO.Text = FormatNumber(TextBox_valorINGRESO.Text, 2)
    ...
    Private Sub Button_aplica_rete_Click(sender As Object, e As EventArgs) Handles Button_aplica_rete.Click
    ...
    Private Sub TextBox_valorgasto_TextChanged(sender As Object, e As EventArgs) Handles TextBox_valorINGRESO.TextChanged
        TextBox_subtotal.Text = TextBox_valorINGRESO.Text
    End Sub
    ...
    Private Sub Button_sel_cuenta_rc_Click(sender As Object, e As EventArgs) Handles Button_sel_cuenta_rc.Click

    End Sub

    Private Sub ComboBox2_SelectionChangeCommitted(sender As Object, e As EventArgs) Handles ComboBox_cxc.SelectionChangeCommitted
        Timer_factura_info.Enabled = True

    End Sub

    Private Sub CargarConceptoDeIngresosToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles CargarConceptoDeIngresosToolStripMenuItem.Click
        ...
        Try
            ...
            Me.ComboBox_tipo_ingreso.DataSource = dT_combo_CUENTAS_CONCEPTO.DefaultView
        ...
        conex.Close()
        ComboBox_tipo_ingreso.DroppedDown = True
    End Sub
    ...
    Private Sub TXT_DOC_CLIENTE_LocationChanged(sender As Object, e As EventArgs)
    ...
    Private Sub TXT_DOC_CLIENTE_OnValueChanged(sender As Object, e As EventArgs) Handles TXT_DOC_CLIENTE.OnValueChanged
        ...
        If IsNumeric(TXT_DOC_CLIENTE.Text) = True Then
    ...
    Private Sub TextBox_valor_OnValueChanged(sender As Object, e As EventArgs) Handles TextBox_valor.OnValueChanged
        If TextBox_valor.Text = "" Then Exit Sub
        totalizar()

    End Sub

    Private Sub ComboBox1_SelectedIndexChanged_1(sender As Object, e As EventArgs) Handles ComboBox_cta_caja_banco.SelectedIndexChanged
    ...
    Private Sub Label_consecutivo_Click(sender As Object, e As EventArgs) Handles Label_consecutivo.Click
    ...
    Private Sub ComboBox_RETENCIONES_SelectionChangeCommitted(sender As Object, e As EventArgs) Handles ComboBox_RETENCIONES.SelectionChangeCommitted
        'RETENCIONES
        Try
            sql = "SELECT porcentaje, NOMBRE FROM impuestos where tipo='VENTAS' and tipoimp='RETENCION' AND CONS='" & ComboBox_RETENCIONES.SelectedValue & "'"
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            da.Fill(dt)
        ...
        RETE_BASE = CDec(TextBox_valorINGRESO.Text) / CDec((CDec(RETE_PORCENTAJE) / 100) + 1)
        RETE_VALOR = TextBox_valorINGRESO.Text - RETE_BASE
    ...
    Private Sub ComboBox_impuestos_SelectionChangeCommitted(sender As Object, e As EventArgs) Handles ComboBox_impuestos.SelectionChangeCommitted
        'IMPUESTOS
        Try
            sql = "SELECT porcentaje, NOMBRE FROM impuestos where tipo='VENTAS' and tipoimp='PORCENTAJE' AND CONS='" & ComboBox_impuestos.SelectedValue & "'"
            da = New MySqlDataAdapter(sql, conex)
            dt = New DataTable
            da.Fill(dt)
        ...
        IMP_BASE = CDec(TextBox_valorINGRESO.Text) / CDec((CDec(IMP_PORCENTAJE) / 100) + 1)
        IMP_VALOR = TextBox_valorINGRESO.Text - IMP_BASE
    ...
    Private Sub Form_rc_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        '        Try
        '            sql = "SELECT * FROM medios_pago
        'inner join cajasybancos on medios_pago.codcuenta=cajasybancos.cons
        'where medios_pago.COBRAR='SI' AND medios_pago.TIPO IN('CAJA','BANCO')"
        ...
        'LOAD CUENTAPUC  CONCEPTO
        Try
            sql = "SELECT cons, documento, CUENTA1 from data_docs where estado='SI' and CODIGO='RC' AND CUENTA1<>''"
            da_combo_CUENTAS_CONCEPTO = New MySqlDataAdapter(sql, conex)
            dT_combo_CUENTAS_CONCEPTO = New DataTable
            da_combo_CUENTAS_CONCEPTO.Fill(dT_combo_CUENTAS_CONCEPTO)
            Me.ComboBox_tipo_ingreso.DataSource = dT_combo_CUENTAS_CONCEPTO.DefaultView
        ...
        'retenciones
        Try
            sql = "SELECT cons, TIPOIMP, porcentaje, nombre FROM impuestos where tipo='VENTAS' and tipoimp='RETENCION'"
    ...
    Private Sub TextBox_valorgasto_LostFocus(sender As Object, e As EventArgs) Handles TextBox_valorINGRESO.LostFocus
        totalizar()

    End Sub

    Private Sub TextBox_valorINGRESO_KeyPress(sender As Object, e As KeyPressEventArgs) Handles TextBox_valorINGRESO.KeyPress
        e.KeyChar = ""
    End Sub

    Private Sub Button_sel_cuenta_rc_MouseDown(sender As Object, e As MouseEventArgs) Handles Button_sel_cuenta_rc.MouseDown
        If e.Button = System.Windows.Forms.MouseButtons.Right = False Then
            Button_sel_cuenta_rc.ContextMenuStrip = ContextMenuStrip_LOAD_PUC
            Me.ContextMenuStrip_LOAD_PUC.Show(Button_sel_cuenta_rc, New Point(Button_sel_cuenta_rc.Width - Button_sel_cuenta_rc.Width, Button_sel_cuenta_rc.Height))
        End If
    End Sub
